...
void get_test_data(DataFrame& df, _Params& params, int length) {
    ...
    PyList_SET_ITEM(param, 4, PyLong_FromLong(30l));
    PyList_SET_ITEM(param, 5, PyFloat_FromDouble(0.0));}
...
int main(int, char**){
    ...
    printf("strategy on_bar() time per loop: %lld ns, total: %lld ms, ", time/1000/length, time/1000000);
